...
from homopt.optim.verbose import maybe_write_row as _maybe_write_verbose_row
...
def _write_stiefel_verbose_row(
    *,
    verbose,
    printed_header,
    iteration,
    ...
    return _maybe_write_verbose_row(
        print,
        enabled=verbose,
...
class StiefelALMEQPGDOptimizer:
    ...
    def solve_result(
        ...
        for outer_iter in range(int(outer_iterations)):
            ...
            eq_violation, ineq_violation, violation = _constraint_violation_summary(self.problem, x)
            printed_verbose_header = _write_stiefel_verbose_row(
                verbose=verbose,
