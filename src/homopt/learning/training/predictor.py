...
from homopt.models.nn import DecisionPredictorNet, load_decision_predictor, save_decision_predictor
from homopt.problems import bind_problem_instance

from .cache import build_training_payload, load_or_train_mapping
from .utils import (
...
def build_predictor_training_payload(problem_args, model_args, train_args, *, ensure_results_save_freq=False):
    """Build the canonical training payload for neural decision predictors."""

    return build_training_payload(
        problem_args,
        model_args,
        train_args,
        problem_key="problem_config",
        model_key="predictor_model_config",
...
def train_decision_predictor(data, args, save_dir, input_tensor=None):
    ...
    supervise_target_raw = paras.get("supervise_target")
    if approach == "supervise" and supervise_target_raw is None:
        raise ValueError("predictor_model_config must set supervise_target for supervised predictor training.")
    supervise_target = None if supervise_target_raw is None else str(supervise_target_raw).strip().lower()

    target_decision = None
    if input_tensor is None and supervise_target in {"dataset", "reference", "sample"}:
        if not hasattr(data, "sample_supervised_instances"):
            raise ValueError(f"{type(data).__name__} does not provide supervised dataset targets.")
        input_tensor, target_decision = data.sample_supervised_instances(
            n_samples=training_sample,
            seed=int(paras.get("seed", 2025)),
            device=runtime_device,
            dtype=runtime_dtype,
        )
        input_tensor = input_tensor.to(device=runtime_device, dtype=runtime_dtype)
        target_decision = target_decision.to(device=runtime_device, dtype=runtime_dtype)
        training_sample = int(input_tensor.shape[0])
    ...
    if supervise_target in {"fixed_center", "center"}:
    ...
    elif supervise_target in {"dataset", "reference", "sample"}:
        if target_decision is None:
            raise ValueError("Dataset supervise_target requires training inputs sampled from the problem dataset.")
        ...
        target_decision, solver_success = _solver_targets(
            ...
            allow_fallback=bool(paras.get("allow_target_fallback", False)),
            fallback_target=fallback_target,
        )
        solver_success_rate = float(solver_success) / max(int(input_tensor.shape[0]), 1)
    elif supervise_target is None and approach == "unsupervise":
        target_decision = None
    else:
    ...
    for iteration in range(pre_training):
        if target_decision is None:
            raise ValueError("pre_training requires a supervised predictor target.")
    ...
    for iteration in range(total_iteration):
        ...
        if target_decision is None:
            supervision_loss = torch.zeros((), device=runtime_device, dtype=runtime_dtype)
        else:
...
def prepare_decision_predictor(
    ...
    *,
    ...
    """Build payload and load/train a neural decision predictor."""
    ...
    payload = build_predictor_training_payload(
        problem_args,
        model_payload,
        train_args,
        ensure_results_save_freq=ensure_results_save_freq,
    )
    model, training_record, model_path, record_path = load_or_train_decision_predictor(
        data,
        payload,
        save_dir,
        retrain=retrain,
