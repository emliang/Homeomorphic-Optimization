...
from .primitives import draw_metric_convergence, draw_objective_residual_landscape, draw_trajectory, draw_unit_ball
from .style import (
    ALGORITHM_COLORS,
...
def _compute_latent_landscape(
    ...
    *,
    ...
    finite = traj[np.isfinite(traj).all(axis=1)] if traj.size else np.empty((0, 2))
    max_abs = max(float(np.max(np.abs(finite[:, :2]))), 1.0) if finite.size else 1.0
    lim = max(1.5, max_abs + 0.15)
