...
def extract_comparison_metrics(summary):
    return {name: summary.get(name) for name in COMPARISON_METRIC_FIELDS}
...
def select_best_comparison_row(rows):
    ...
    return min(
        primary_rows,
        key=lambda row: float("inf") if row.get("objective_mean") is None else float(row["objective_mean"]),
    )
