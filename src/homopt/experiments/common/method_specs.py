...
from .config import merged, reject_iteration_budget_keys
...
def build_convex_penalty_method_kwargs(
    ...
    *,
    ...
    return {
        ...
        "inner_min_lr": common.get("inner_min_lr", common["min_lr"]),
        "inner_stepsize_rule": "constant",
        "inner_solver": "gd",
        "inner_proximal_update_iterations": 1,
        "inner_proximal_coef": 0.0,
...
def normalize_jcc_lagrangian_config(
    *,
    ...
    if lagrangian_baseline_config:
        lag_args = normalize_penalty_method_config(
            penalty_method_args=lag_args,
            penalty_method_config=lagrangian_baseline_config,
        )
        if max_iterations is not None:
            lag_args["outer_iterations"] = int(max_iterations)
    return lag_args
