...
from homopt.experiments.common.comparison import make_comparison_row, timing_extras_from_iter_time


def build_jcc_method_row(problem, *, route, method, objective, x_opt, runtime_total, status, **extras):
    explicit_feasible = extras.pop("feasible", None)
    if extras.get("chance_feasibility_rate") is not None:
        ...
        feasible = bool(violation <= 1e-12 if explicit_feasible is None else explicit_feasible)
        return make_comparison_row(
            ...
            inequality_violation=violation,
            **extras,
        )
    if x_opt is None:
    ...
    if explicit_feasible is not None:
        feasible = bool(explicit_feasible)
    return make_comparison_row(
        route=route,
        method=method,
        objective=objective,
...
def build_jcc_iterative_method_rows(problem, results):
    ...
    for method in ("INN-PGD", "ALM", "Penalty", "Prox-Penalty"):
        ...
        extras = timing_extras_from_iter_time(
